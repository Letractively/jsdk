 ...
 * @modified: 2011.8.1
 */
js.ui.menu.PopupMenuLite=function(owner,mode){
	this._id="";
	this._owner=null;
	...
	this._version="0.2 Alpha 20110802";
	this._PopupMenuLite(owner,mode);
}
...
with(_$class){
	...
	prototype._PopupMenuLite=function(owner,mode){
		var thisObj=this;
		this._owner=owner;
	...
	addMethod(false,true,"addItem",function(sTitle,fnAction){	
		var Menu=this.getClass().Menu;
		var Item=this.getClass().Item;
		if(!this._mainMenu){
			this._mainMenu=new Menu(this);
		}
		var newItem=new Item(this._mainMenu,sTitle,fnAction);
		this._mainMenu.addItem(newItem);
		return(newItem);
	});
	addMethod(false,true,"show",function(iWidth,aCoordinate){
		if(this._mainMenu) this._mainMenu.show(iWidth,aCoordinate);
	});
	...
	addEventListener(true,false,"onFirstInstanced",function(oAppMenu){
		...
		document.body.attachEvent("onclick",function(event){
			...
			if(typeof(data)=="object"&&data.activeApp==thisObj){
				...
				if(thisObj.getActiveAppMenu()) thisObj.getActiveAppMenu().hide();
			}
		});
	});
...
with(_$class.Menu){
	...
	**/
	...
	prototype.show=function(iWidth,aCoordinate){
		var thisObj=this;
		var iLeft=0,iTop=0,iHeight=0;
	
		window.event.returnValue=window.event.result={
		...
		iWidth=Math.max(this._getMinWidth(),iWidth||0);
		iHeight=this._getMinHeight();
		this._htmlElement.style.left=iLeft+"px";
		this._htmlElement.style.top=iTop+"px";
		this._htmlElement.style.width=(Browser.Engine.trident?iWidth:(iWidth-2))+"px";
		this._htmlElement.style.height=(Browser.Engine.trident?iHeight:(iHeight-2))+"px";
		this._htmlElement.style.display="";
		this._htmlContainer.style.width=(Browser.Engine.trident?(iWidth-2):(iWidth-6))+"px";
		this._htmlContainer.style.height=(Browser.Engine.trident?(iHeight-2):(iHeight-6))+"px";
		this._showSepLine();
	...
	prototype._showSepLine=function(){
		for(var i=0,iLen=this._items.length-1;i<iLen;i++){
			if(this._items[i].getTitle()=="-"){
				this._items[i]._htmlElement.style.display="";
			}
		}
	...
	**/
	prototype._toHtmlElement=function(){
		...
		with(this._htmlElement){
			setAttribute("style","-moz-user-select:none");
			style.position="absolute";
			style.borderLeft="solid 1px #D0CFCB";
			style.borderTop="solid 1px #D0CFCB";
			style.borderRight="solid 1px #40403E";
			style.borderBottom="solid 1px #40403E";
			style.cursor="default";
			with(this._htmlContainer=appendChild(document.createElement("div"))){
				id="content";
				style.width="100%";
				style.height="100%";
				style.borderLeft="solid 1px #FFFFFB";
				style.borderTop="solid 1px #FFFFFB";
				style.borderRight="solid 1px #83807B";
				style.borderBottom="solid 1px #83807B";
				style.padding="1px";
			...
			onclick=function(event){
				...
				event.returnValue=event.result={
					activeAppMenu : thisObj.getParentApp()
				}
			}
		}
	}
	...
	**/
	...
	prototype._getMinWidth=function(){
		var width=0;
		for(var i=0;i<this._items.length;i++){
			var sTitle=Object(this._items[i].getTitle());
	...
	prototype._getMinHeight=function(){
		var height=6;
		for(var i=0;i<this._items.length;i++){
			if(this._items[i].getTitle()=="-") height+=6;
			else height+=18;
		}
		return height;
	}
 ...
 * @modified: 2011.7.29
 */
_$class.Item=function(parentMenu,sTitle,fnAction){
	this._id="";
	this._title="";
	...
	this._subMenu=null;	
	this._htmlElement=null;	
	this._Item(parentMenu,sTitle,fnAction);
}
with(_$class.Item){
	$name="Item";
	$extends(Object);
	...
	prototype._Item=function(parentMenu,sTitle,fnAction){
	...
	prototype.refresh=function(){
		if(!this._htmlElement) return;
		this._htmlElement.style.color=this._enabled?"black":"gray";
	...
	prototype._toHtmlElementForStd=function(){
		...
		with(this._htmlElement=this.getParentMenu()._htmlContainer.appendChild(
			...
			with(appendChild(document.createElement("span"))){
				...
				innerHTML="<span style=\"padding:2px 2px 0px 2px;height:14px;line-height:14px;display:inline-block\">"+this._title+"</span>";
			}
			with(appendChild(document.createElement("span"))){
				id="sub";
				style.width="12px";
	...
	prototype._toHtmlElementForSep=function(){
		with(this._htmlElement=this.getParentMenu()._htmlContainer.appendChild(
			document.createElement("div"))){
			style.width="100%";
			style.height=Browser.Engine.trident?"2px":"0px";
			style.margin="2px 0px";
			style.fontSize="9pt";
			style.borderTop="solid 1px #848081";
			style.borderBottom="solid 1px #FFFEFA";
			style.overflow="hidden";
			style.display="none";
		}
	}
	...
	prototype._onActivate=function(oEvent){
		with(this._htmlElement){
			if(this._enabled) style.color="white";
			style.backgroundColor="#0054E3";
		}
		this.getParentMenu()._activeItem=this;
	}
	prototype._onDeactivate=function(oEvent){
		with(this._htmlElement){
			if(this._enabled) style.color="";
			style.backgroundColor="";
		}
		this.getParentMenu()._activeItem=null;
	}
	prototype._onAction=function(oEvent){
