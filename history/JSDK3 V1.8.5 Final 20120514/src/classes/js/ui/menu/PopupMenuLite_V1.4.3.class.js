...
with(_$class){
	...
	_$class._styleLib={
		"classic" : Engine.runtimeEnvironment.getResPath("js.ui.menu")+"/PopupMenuLite/Classic.css",
		"xp": Engine.runtimeEnvironment.getResPath("js.ui.menu")+"/PopupMenuLite/XP.css"
	}
...
with(_$class.Menu){
		...
		if(aCoordinate){
			...
			switch(this.getParentApp().getMode()){
				...
				case 1:		//dropdown menu
					...
					iWidth=Math.max(this._getMinWidth(),iWidth||this.getParentApp().getOwner().offsetWidth);
					break;
			}
		}
	...
	**/
	prototype._toHtmlElement=function(){
		...
		with(this._htmlElement){
			if(Browser.Engine.trident) setAttribute("className","OutsideBox PopupMenu");
			else setAttribute("class","OutsideBox PopupMenu");
			with(this._htmlContainer=appendChild(document.createElement("div"))){
				if(Browser.Engine.trident) setAttribute("className","InsideBox PopupMenuContent");
				else setAttribute("class","InsideBox PopupMenuContent");
				id="content";
			}
	...
	**/
	...
	prototype._getMinWidth=function(){
		var width=0;
		for(var i=0;i<this._items.length;i++){
			if(this._items[i].getIsHidden()) continue;
...
with(_$class.Item){
	...
	prototype._toHtmlElementForStd=function(){
		...
		with(this._htmlElement=this.getParentMenu()._htmlContainer.appendChild(
			...
			if(Browser.Engine.trident) setAttribute("className","PopupMenuItem");
			else setAttribute("class","PopupMenuItem");
	...
	prototype._toHtmlElementForSep=function(){
		with(this._htmlElement=this.getParentMenu()._htmlContainer.appendChild(
			document.createElement("div"))){
			if(Browser.Engine.trident) setAttribute("className","PopupMenuSepLine");
			else setAttribute("class","PopupMenuSepLine");
		}
	}
